...
if($symbol -eq $null){
    ...
    if(($percent -gt .5) -and ($day.Close -gt $50day[0].close) -and ($day.open -le ($line.buypoint * 1.03)) -and ($day.close -gt $line.buypeak)){
        $array = @()
...
$collection | Export-Csv -NoTypeInformation missedvoldaybuypoints.csv
